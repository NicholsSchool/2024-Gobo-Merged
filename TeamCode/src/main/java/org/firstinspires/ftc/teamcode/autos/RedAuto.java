...
public class RedAuto extends LinearOpMode implements DriveConstants, ArmConstants, TeleopConstants {
    enum PropZones {
        LEFT,
        CENTER,
        RIGHT
    ...
    public void runOpMode() {
        ...
        if(bestRec != null) {
            ...
            if (point < 450.0) {
                ...
                purplePixelAngle = -105;
            } else {
                propZone = PropZones.RIGHT;
                purplePixelAngle = -125;
            }
        }
        else {
            propZone = PropZones.LEFT;
            purplePixelAngle = 0.0;
        }

        double[] scanCoords = propZone.equals(PropZones.LEFT) ? new double[]{35.0, 38.0} : new double[]{36.0, 38.0};
